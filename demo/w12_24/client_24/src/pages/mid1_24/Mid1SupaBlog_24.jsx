...
const Mid1SupaBlog_24 = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        let { data, error } = await supabase.from('blog_24').select('*');
  ...
  const { mutate: deleteBlog } = useMutation({
    mutationFn: async (id) => {
      try {
        const { error } = await supabase.from('blog_24').delete().eq('id', id);
      } catch (error) {
        console.log(error);
      }
    },
  ...
  const { mutate: clearAll } = useMutation({
    mutationFn: async () => {
      try {
        const { error } = await supabase.from('blog_24').delete().gt('id', 0);
      } catch (error) {
        console.log(error);
      }
    },
...
export default Mid1SupaBlog_24;
